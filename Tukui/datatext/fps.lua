...
if TukuiCF["datatext"].fps_ms and TukuiCF["datatext"].fps_ms > 0 then
	...
	local function Update(self, t)
		int = int - t
		if int < 0 then
			Text:SetText(floor(GetFramerate())..panelcolor.." fps|r & "..select(3, GetNetStats())..panelcolor.." ms")
			int = 1
		end	
	end
	...
	local function formatMem(memory, color)
		...
		if memory > 999 then
			...
			if mem % 1 == 0 then
				return mem..string.format(".0"..panelcolor.." %smb%s", unpack(statColor))
			else
				return mem..string.format(""..panelcolor.." %smb%s", unpack(statColor))
			end
		else
			local mem = floor(memory * mult + 0.5) / mult
				if mem % 1 == 0 then
					return mem..string.format(".0"..panelcolor.." %skb%s", unpack(statColor))
				else
					return mem..string.format(""..panelcolor.." %skb%s", unpack(statColor))
				end
		end
	end
