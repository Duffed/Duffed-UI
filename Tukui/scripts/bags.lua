...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		if TukuiCF["chat"].leftchatborder == true then
			f:SetPoint("BOTTOMRIGHT", ChatBG1, "TOPRIGHT", 0,TukuiDB.Scale(5))
		else
			f:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 145,7)
		end
	else
		if TukuiCF["chat"].rightchatborder == true then
			f:SetPoint("BOTTOMLEFT", ChatBG2, "TOPLEFT", 0,TukuiDB.Scale(5))
		else
			f:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", -145,7)
		end
	end
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:HookScript("OnShow", function(self)
		...
		self:ClearAllPoints()
		self:SetPoint("TOPRIGHT", StuffingFrameBags, "TOPLEFT", TukuiDB.Scale(-1), TukuiDB.Scale(5))
		TukuiDB.CreateShadow(keybackdrop)
	end)
end
