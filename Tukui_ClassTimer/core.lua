...
if ( TukuiCF and TukuiCF["media"] and TukuiCF["media"]["uffont"] ) then
	...
	MASTER_FONT = { TukuiCF["media"]["uffont"], 12 };

	-- Sets font for stack count
	STACKS_FONT = { TukuiCF["media"]["uffont"], 11 , "THINOUTLINE"};
else
	-- Sets font for all texts
	MASTER_FONT = { [=[Interface\Addons\Tukui\media\Russel Square LT.ttf]=], 12 };

	-- Sets font for stack count
	STACKS_FONT = { [=[Interface\Addons\Tukui\media\Russel Square LT.ttf]=], 11 };
end
...
local CLASS_FILTERS = {
		...
		PRIEST = { 
            ...
            player = {
				...
				CreateSpellEntry( 87118 ), -- Evangelism
				CreateSpellEntry( 81660 ), -- Dark Evangelism
            },
            procs = {
...
do
	...
	CreateFramedTexture = function( parent )
		...
		texture:SetPoint( "TOPLEFT", result, "TOPLEFT", 3, -3 );
		texture:SetPoint( "BOTTOMRIGHT", result, "BOTTOMRIGHT", -3, 3 );
...
do
	...
	do
		...
		CreateAuraBar = function( parent )
			...
			icon:SetPoint( iconAnchor1, result, iconAnchor2, iconOffset * -5, 3 )
			...
			stacks:SetPoint( "BOTTOMRIGHT", icon, "BOTTOMRIGHT", -1, 3 );
			result.stacks = stacks;
			
			local bar = CreateFrame( "StatusBar", nil, result, nil );
			bar:SetStatusBarTexture( [=[Interface\Addons\Tukui\media\textures\normTex]=] );
			bar:SetPoint( "TOPLEFT", result, "TOPLEFT", 9, 0 );
	...
	local SetAuraBar = function( self, index, auraInfo )
		...
		if ( line == nil ) then
			...
			if ( index == 1 ) then
				line:SetPoint( "TOPLEFT", self, "BOTTOMLEFT", 13, BAR_HEIGHT);
	...
	CreateAuraBarFrame = function( dataSource, parent )
		...
		TukuiDB.SetTemplate(background)
		background:SetPoint( "TOPLEFT", result, "TOPLEFT", 20, 2 );
		background:SetPoint( "BOTTOMRIGHT", result, "BOTTOMRIGHT", 2, -2 );
		result.background = background;
		
		local border = CreateFrame( "Frame", nil, result, nil );
		border:SetFrameStrata("BACKGROUND")
		border:SetPoint( "TOPLEFT", result, "TOPLEFT", 21, 1 );
		border:SetPoint( "BOTTOMRIGHT", result, "BOTTOMRIGHT", -1, -1 );
		border:SetBackdrop {
		  edgeFile = TukuiCF["media"].blank, edgeSize = 1,
		...
		TukuiDB.CreatePanel(iconborder,1,1,"TOPLEFT", result, "TOPLEFT", -2, 2)
		iconborder:SetPoint("BOTTOMRIGHT", result, "BOTTOMLEFT", BAR_HEIGHT+2, -2)
